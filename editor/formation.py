...
class Formation():
    ...
    # ── Fixed structural constants (same for all PS2 WE/PES titles) ───────────
    jobs_count     = 6    # tactical job assignment slots per formation
    settings_count = 4    # formation-level setting fields
    player_count   = 11   # players per formation slot

    # ── Config-injected class attributes ──────────────────────────────────────
    # Sentinel defaults — real values set by OptionFile.__init__ from YAML.
    # Formation: Size / Formation: Alt Size
    size     = 364  # Formation: Size
    alt_size = 82   # Formation: Alt Size
    ...
    def jobs(self):
        ...
        return struct.unpack(
            "<%dB" % self.jobs_count,
            self.team.of.data[job_address: job_address + self.jobs_count]
        )

    @jobs.setter
    def jobs(self, new_val: tuple):
        job_address = self.address + 111
        self.team.of.data[job_address: job_address + self.jobs_count] = struct.pack(
            "<%dB" % self.jobs_count, *new_val
        )
    ...
    def form_values(self):
        val = struct.unpack(
            ...
            self.team.of.data[
                self.coordinates_address: self.coordinates_address + self.form_data_len
            ]
        )
    ...
    def form_values(self, new_val: tuple):
        self.team.of.data[
            self.coordinates_address: self.coordinates_address + self.form_data_len
        ] = struct.pack("<%dB" % self.form_data_len, *new_val)
        self.form_data[-1] = new_val

    @property
    def roles(self):
        return [
            self.position_to_string(val)
            for val in self.form_values[(self.player_count - 1) * 2:]
        ]
