...
class Emblem:
    # ── Fixed image-format constants (same for all PS2 WE/PES titles) ─────────
    empty_idx_value = 0x99  # value in club record meaning no custom emblem
    bpp_128         = 8     # bits per pixel, high-res layer (256-colour)
    bpp_16          = 4     # bits per pixel, thumbnail layer (16-colour)
    width           = 64    # pixel canvas width (both layers)
    height          = 64    # pixel canvas height

    # ── Config-injected class attributes ──────────────────────────────────────
    # Sentinel defaults — real values set by OptionFile.__init__ from YAML.
    total_128 = 50    # Emblem: Total
    size_128  = 5184  # Emblem: Size 128
    size_16   = 2176  # Emblem: Size 16
